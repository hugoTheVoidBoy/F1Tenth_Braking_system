...
I implemented an Emergency Braking System feature by adding safety_node.py to the open-source simulator F1Tenth Gym. This stops the vehicle when too close to a wall (iTTC < 1). The algorithm I used is
 iTTC = range / (-range)’
...
Velocity decreased to 0.0 due to iTTC implementation.
...
5.	Open a browser, the simulator will be running on http://localhost:8080/vnc.html. Click the CONNECT button.

6.	Open another terminal, run this bash to initiate the container:
$ docker exec -it f1tenth_gym_ros-sim-1 /bin/bash

7.	After the docker container is activated, bash this to activate keyboard movement control:
$ source /opt/ros/foxy/setup.bash
$ source install/setup.bash
...
Additional Information: Docker Compose runs the docker-compose.yml to launch the file onto noVNC on the local port. Preliminary objects setup are done in the config/sim.yaml file to set up the topics, set up the map based on a .png file and the location (x,y,z) of the car object on png. Then 2D is converted to 3D using transforms3d Python library in the gym_bridge.py .
